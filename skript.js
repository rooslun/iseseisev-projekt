...
function addShop(event) {
	...
	if (shopItems.indexOf(shopInput.value) === -1) {
		const newShop = document.createElement('li');
		newShop.innerText = shopInput.value;
		newShop.classList.add('shop-item'); // lisab uue itemi
	...
	const test = shopList.querySelectorAll("li")
	const test2 = document.querySelectorAll(".shop");

	shopItems.push(shopInput.value)
	shopInput.value = "";
	...
	test.forEach(text => {
		...
		test2.forEach(item2 => {
			const li = item2.querySelector('li')
			if (li.innerText === text.innerText) {
				const span = item2.querySelector('span')
